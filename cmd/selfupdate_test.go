...
// github.com/giantswarm/selfupdate-cosign. What follows checks that mcp-capi
...
func (r fakeRelease) GetURL() string {
	return "https://github.com/giantswarm/mcp-capi/releases/tag/" + r.tag
}
...
func binaryAsset() string {
	name := "mcp-capi-" + runtime.GOOS + "-" + runtime.GOARCH
...
func selfUpdateFixture(t *testing.T, src *fakeSource) (string, []byte) {
	t.Helper()
	installed := []byte("the mcp-capi that is installed right now")
	exe := filepath.Join(t.TempDir(), "mcp-capi")
...
func TestRunSelfUpdateRefusesAReleaseWithoutASignatureBundle(t *testing.T) {
	src := &fakeSource{
		...
		assets:  map[int64][]byte{1: []byte("a newer mcp-capi, unsigned")},
	}
...
func TestRunSelfUpdateInstallsAVerifiedReleaseInPlace(t *testing.T) {
	...
	src := &fakeSource{
		...
		assets: map[int64][]byte{1: []byte("a newer mcp-capi"), 2: []byte("its bundle")},
	}
	...
	got, err := os.ReadFile(exe) //nolint:gosec // the test's own temp file
	if err != nil || string(got) != "a newer mcp-capi" {
